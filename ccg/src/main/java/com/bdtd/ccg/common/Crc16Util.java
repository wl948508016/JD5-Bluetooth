...
public class Crc16Util {
     ...
     * @return 源数据 + CRC16校验码
     */
    public static String getData(String strings) {
        byte[] data = new byte[]{};
        char[] split = strings.toCharArray();
        for (int i = 0; i < split.length; i += 2) {
            int x = Integer.parseInt(String.valueOf(split[i]) + split[i + 1], 16);
            byte n = (byte) x;
        ...
        return HexUtil.byteToHexStr(getData(data));
    }
